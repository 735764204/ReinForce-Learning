    ...
    def loop(self):
        while True:
            ...
            if self.autoMC:
                ...
                if len(self.agent.A_MC) > 10 and abs(self.agent.A_MC[-1] - self.agent.A_MC[-2]) < 0.0003:
                    plt.plot(range(len(self.agent.A_MC)), self.agent.A_MC)
                    plt.show()
                    sys.exit()
                # if len(self.agent.B_MC) > 10 and abs(self.agent.B_MC[-1] - self.agent.B_MC[-2]) < 0.00007:
                #     plt.plot(range(len(self.agent.B_MC)), self.agent.B_MC)
                #     plt.show()
                #     sys.exit()
            if self.autoTD:
                #TD Learning
                ...
                if len(self.agent.A_TD) > 20000 and abs(self.agent.A_TD[-1] - self.agent.A_TD[-2]) < 0.00005:
                    plt.plot(range(len(self.agent.A_TD)), self.agent.A_TD)
                    plt.show()
                    sys.exit()
                # if len(self.agent.B_TD) > 100 and abs(self.agent.B_TD[-1] - self.agent.B_TD[-2]) < 0.0007:
                #     plt.plot(range(len(self.agent.B_TD)), self.agent.B_TD)
                #     plt.show()
                #     sys.exit()
            ...
            if self.autoQL:
                ...
                self.agent.Q_run(50)
                if len(self.agent.A_Q[0]) > 5000:
                    plt.plot(range(len(self.agent.A_Q[0])), self.agent.A_Q[0])
                    plt.plot(range(len(self.agent.A_Q[1])), self.agent.A_Q[1])
                    plt.show()
                    sys.exit()
                # if len(self.agent.B_Q[0]) > 5000:
                #     plt.plot(range(len(self.agent.B_Q[0])), self.agent.B_Q[0])
                #     plt.plot(range(len(self.agent.B_Q[1])), self.agent.B_Q[1])
                #     plt.show()
                #     sys.exit()
            ...
            if self.autoPlay:
                if self.game.game_over() or self.game.stand:
                    ...
                    self.plot.append(self.game.winNum/ (self.game.winNum + self.game.loseNum))
                    if len(self.plot) > 10000:
                        plt.plot(range(len(self.plot)), self.plot)
                        plt.show()
                        sys.exit()
                    self.game.reset()
